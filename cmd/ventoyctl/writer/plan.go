...
func BuildWritePlan(m ventoy.WriteMap, targetBytes uint64) (WritePlan, error) {
	if err := validateMap(m, targetBytes); err != nil {
		return WritePlan{}, err
	}
	p1, p2 := m.PartitionTable.Partitions[0], m.PartitionTable.Partitions[1]
	targetSectors := targetBytes / ventoy.SectorSize
	...
	ranges := []CopyRange{
		{ID: "patch-mbr", Source: "__PATCHED_MBR__", SrcOff: 0, DstOff: 0, Length: ventoy.SectorSize},
		{ID: "pre-p1-tail", Source: "__IMAGE__", SrcOff: ventoy.SectorSize, DstOff: ventoy.SectorSize, Length: m.DerivedZones.PreP1.LengthBytes - ventoy.SectorSize},
		{ID: "p2-vtoyefi", Source: "__IMAGE__", SrcOff: p2.OffsetBytes, DstOff: newP2Start * ventoy.SectorSize, Length: p2.LengthBytes},
	}
	...
	return WritePlan{TargetBytes: targetBytes, P1: p1, P2: p2, NewP1Size: newP1Size, NewP2Start: newP2Start, Ranges: ranges, WriteBytes: writeBytes}, nil
}
...
func PatchedMBR(imagePath string, plan WritePlan) ([]byte, error) {
	...
	mbr := make([]byte, ventoy.SectorSize)
...
func PrintPlan(plan WritePlan, imagePath, rawDisk string) {
	...
	fmt.Printf("p1_new_size=%s (%d sectors)\n", HumanBytes(plan.NewP1Size*ventoy.SectorSize), plan.NewP1Size)
	fmt.Printf("p2_new_start_sector=%d\n", plan.NewP2Start)
	fmt.Printf("p2_size=%s (%d sectors)\n", HumanBytes(plan.P2.SizeSectors*ventoy.SectorSize), plan.P2.SizeSectors)
...
func bestBlockSize(srcOff, dstOff, length uint64) uint64 {
	for _, candidate := range []uint64{4 * 1024 * 1024, 1024 * 1024, 64 * 1024, ventoy.SectorSize} {
		if srcOff%candidate == 0 && dstOff%candidate == 0 && length%candidate == 0 {
			return candidate
		}
	}
	return ventoy.SectorSize
}
